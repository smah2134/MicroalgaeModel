...
load 'filename'                      %% Change to file name
LightPAR= ;                          %% Specify PAR data
...
timestep_s =timestep_AH1*24*3600;                                                    %Time step size (s)               
...
[R,dx,dtheta,dthetaR,L,x,rho,dx1,Rv,Rstep,theta,V,x_i,y_i,V_total,thetasteps] = ReactorGeometry_Tube;                                                    %Load tube geometry function
...
for t=1:1:timesteps_total
    ...
    %% Light Distribution Model
       ...
       % Beer-Lambert Model
     [I]=LightModel_Tube_BeerLambert(X(t),I_0(t),R,x_i,y_i);


    %% Growth Kinetics Model


        [mu_max,mu,X_dv] = GrowthModel_simpleMonod_CTMI_Resp(T_R(t),I,I_0(t),timestep,X(t));
